...
public class python3 extends AppCompatActivity {
    ...
    String[][] options = {
            {"var x = 10", "x = 10", "int x = 10", "let x = 10"},
            {"array", "tuple", "map", "setlist"},
            {"6", "8", "9", "Error"},
            {"strlen()", "len()", "size()", "length()"},
            {"for (i = 0; i < 10; i++)", "for i in range(10):", "for i=0 to 10", "for (i < 10):"},
            {"{1, 2, 3}", "[1, 2, 3]", "(1, 2, 3)", "<1, 2, 3>"},
            {"func", "function", "def", "lambda"},
            {"<class 'float'>", "<class 'int'>", "<class 'double'>", "<class 'decimal'>"},
            {"a == b", "a = b", "a === b", "a equals b"},
            {"try-except", "catch-finally", "throw-catch", "raise-handle"}
    };

    String[] correctAnswers = {"x = 10", "tuple", "8", "len()", "for i in range(10):", "[1, 2, 3]", "def", "<class 'float'>", "a == b", "try-except"};
    ...
    private void setUpListeners() {
        ...
        submitButton.setOnClickListener(v -> {
            saveUserAnswer();
            calculateScore();
            Intent intent = new Intent(python3.this, resultpy3.class);
